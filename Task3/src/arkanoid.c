    ...
    while (true) {
    	{
			...
			while (true) {
				// TODO collision makes most sense here.
				// i've already modified the function signature here including a return of "-1" for "no collision" and "-2" for "paddle collision"
			    ballPhysicsStep(&playerBall, &playerPaddle, blocks);
			    finishBallMovement(&playerBall, blocks);
			    // TODO do things on hit block

			    // TODO the level should end if conditions are met (and if the ball hits the paddle)
			    ...
			    // TODO render score, combo, level
				// ST7735_WriteString(2, 150, Font4x6, ???, ST7735_CYAN, ST7735_BLACK);

				sleep_us(1000000 / 60);
			}
		}

		// TODO level end code here
	}
}